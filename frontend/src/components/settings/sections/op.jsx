...
const modelPresets = ["Flux Schnell"];
			...
			this.section(
				...
				[
					this.textField(
						...
						" {Subject}  Leave it blank to generate random scene without the subject.",
						false,
						10
					),
...
if (
	promptsSourceInput[0][0] == 1 &&
	configuration.model.includes("flux_1_dev")
) {
	requestFromUser(`Flux Auto Workflow ${version}`, "Exit", function () {
		return [
			this.section(
...
function generatePrompt() {
	...
	if (rand < 0.1) {
		...
		randomSubject = getRandom(allPeople);
		switch (selectedScene) {
			case 0:
				if (dailyScenes.includes(sceneArray)) {
		...
		if (Math.random() < 0.3) {
			const name = getRandom(allPeople);
	...
	const w = randomClothes == "" ? "" : " wearing nothing at all";

	var buildPrompt = "";
	switch (creativeMode) {
		case 0:
			buildPrompt = `${randomStyle}, ${view}, ${randomLight}. ${randomSubject}${w}, ${randomAction} ${randomScene}.`;
			break;
		case 1:
			buildPrompt = `${randomStyle}, ${view}, ${randomLight}. ${randomSubject}${w}$${randomScene}.`;
			break;
		case 2:
			...
			return [
				this.section(
					...
					[
						...
						this.slider(
							batchCount,
							this.slider.fractional(0),
							1,
							maxCount,
							"❖  Batch count of each prompt"
						),
					]
				),
			];
...
var titleInfo = "";
if (workflow == 0 || workflow == 1) {
	titleInfo = `   ⎟   Prompts: ${promptsCount}  •  Images: ${
		batchCount * promptsCount
	}`;
} else {
...
const isFluxDownload = pipeline.areModelsDownloaded([
	"FLUX.1 [dev] (Exact)",
	"FLUX.1 [dev]",
	"FLUX.1 [dev] (8-bit)",
]);
	...
	function () {
		...
		if (workflow == 0) {
			...
			widget.push(
				...
				this.section("❖  Outpainting Composition Position Offset", "", [
					this.segmented(0, ["Above", "Below", "Center", "Left", "Right"]),
					this.slider(
						0.5,
						this.slider.fractional(1),
						0.2,
						0.8,
						"⊕ ⊖   Zoom Ratio"
					),
...
if (workflow != 1) {
	if (!unofficialFlux) {
		const isDownload = pipeline.areModelsDownloaded([
			"FLUX.1 [dev] (Exact)",
			"FLUX.1 [dev]",
			"FLUX.1 [dev] (8-bit)",
		]);
...
if (workflow == 0) {
		...
		for (var i = 0; i < batchCount; i++) {
			...
			if (loras.length > 0) {
				for (var x = 0; x < loras.length; x++) {
					...
					if (
						loraName.file == "flux.1__dev__to__schnell__4_step_lora_f16.ckpt"
					) {
						loras.splice(x, 1);
						x--;
					}
			...
			if (mode == 0) {
				console.log(
					`🟢 Speed Mode ‣ Running the Flux Dev   ⚙︎ Image batch progress ‣ ${completedBatches}/${totalBatches}${eTime}`
				);
				schnellLora = loras;
				schnellLora.push({
				...
				pipeline.run({
					...
					prompt: promptsArray[s],
					negativePrompt: null,
				});
			} else if (mode == 1) {
				console.log(
					`🟠 Balance Mode ‣ ❶ Running the Flux Dev    ⚙︎ Image batch progress ‣ ${completedBatches}/${totalBatches}${eTime} `
				);
				schnellLora = loras;
				schnellLora.push({
					file: "flux.1__dev__to__schnell__4_step_lora_f16.ckpt",
				...
				console.log(
					`🟠 Balance Mode ‣ ❷ Refining the image    ⚙︎ Image batch progress ‣ ${completedBatches}/${totalBatches}${eTime} `
				);
				if (detail == 0) {
					configuration.loras = [];
					configuration.sampler = 15;
				...
				console.log(
					`🔴 Quality Mode ‣ Running the Flux Dev    ⚙︎ Image batch progress ‣ ${completedBatches}/${totalBatches}${eTime}`
				);
				configuration.controls = controls;
				configuration.sampler = 15;
				configuration.strength = 1.0;
	...
	if (imgFiles.length == 0) {
		...
		for (var i = 0; i < imgFiles.length; i++) {
			...
			let info = `Expanding the images from folder   ⚙︎ Image batch progress ‣ ${
				i + 1
			}/${imgFiles.length}${eTime}`;
			if (processingMode == 1) {
			...
			info = `Refining the images from folder   ⚙︎ Image batch progress ‣ ${
				i + 1
			}/${imgFiles.length}${eTime}`;
...
function refine(info, srcPrompt) {
	...
	if (capture) {
		console.log(
			`🟢 Capturing Image Description ‣ Running the MoonDream  ⚠️ Please do not move the canvas and other operations while capturing the image description.`
		);
		const answer = canvas.answer(
			"moondream2/20240520",
			"Describe this image, do not exceed 50 words."
		);
		if (answer) {
			p = answer;
		}
	...
	if (detail == 0) {
		...
		configuration.loras = [
			{ file: "flux.1__dev__to__schnell__4_step_lora_f16.ckpt", weight: 0.35 },
		];
		configuration.sampler = 10;
		configuration.shift = 1.0;
		configuration.guidanceScale = 3.5;
		...
		configuration.loras = [
			{ file: "flux.1__dev__to__schnell__4_step_lora_f16.ckpt", weight: 1.0 },
		];
		configuration.sampler = 10;
		configuration.shift = 1.0;
...
function outpainting(info, srcPrompt) {
	...
	const isDownload = pipeline.areModelsDownloaded([
		...
		"Xinsir Union ProMax (SDXL, ControlNet)",
	]);
	if (!isDownload[0]) {
	...
	canvas.canvasZoom = parseFloat(zoom.toFixed(1));
	let x = 0;
	let y = 0;
	switch (positionOffset) {
		case 0:
			...
			y =
				-imageHeight / zoom / 2 +
				(imageHeight / 2 + imageHeight * (1 - zoom) * (1 - zoom));
			break;
		case 1:
			x = -imageWidth / zoom / 2 + imageWidth / 2;
			y =
		...
		case 3:
			x =
				-imageWidth / zoom / 2 +
				(imageWidth / 2 + imageWidth * (1 - zoom) * (1 - zoom));
			y = -imageHeight / zoom / 2 + imageHeight / 2;
			break;
		case 4:
			x =
			...
			if (width > maxSize || height > maxSize) {
				if (width > height) {
					...
					configuration.height =
						Math.round(Math.round((height / width) * maxSize) / 64) * 64;
				} else {
					configuration.height = maxSize;
					configuration.width =
						Math.round(Math.round((width / height) * maxSize) / 64) * 64;
				}
			}
			break;
		default:
...
function decodeExif(binary) {
	const userCommentMatch = binary.match(
		/<exif:UserComment>[\s\S]*?<rdf:li xml:lang="x-default">([\s\S]*?)<\/rdf:li>/
	);
...
function estimateTime(totalStartTime, completedBatches, totalBatches) {
	...
	const estimatedRemainingTime = Math.floor(
		averageTimePerBatch * remainingBatches
	);
...
if (workflow != 2) {
	msg =
		"Next, you can select the Image Processor workflow to Refine or Outpainting the images.";
} else {
	msg =
		"If you are not satisfied with the results, you can adjust the options to get different Refine or Outpainting result.";
}
